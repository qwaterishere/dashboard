"""Тесты сборки страницы «Фудкост»: правило paid>0 & cost>0, скидки, потери."""
...
CURRENT = [
    ...
    # бизнес-ланч без техкарты: в выручке есть, в знаменатель фудкоста не входит
...
def test_rule3_blocks_and_lfl(session, restaurant):
    ...
    assert by_key['k'].goal is None
    assert by_key['b'].revenue == 280 and by_key['b'].cost == 48
    assert by_key['b'].goal is None
