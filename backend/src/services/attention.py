...
from src.services.analytics.pace import is_pace_risk
...
from src.services.foodcost import compliment_sums, foodcost_totals
from src.services.stock import SnapshotNotFound, stock_negative
from src.services.targets import load_foodcost_goals, month_revenue_plan_configured
...
def _foodcost_domain(
    ...
    try:
        ...
        goals = load_foodcost_goals(
            session, restaurant_id, year, month, unit_revenues={},
        )
        # totals.goal уже weighted при наличии юнитов; иначе fallback из goals.
        clean_goal = totals.goal if totals.goal is not None else goals.totals_goal_pct
...
def _targets_domain(
    ...
    try:
        configured = month_revenue_plan_configured(
            session, restaurant_id, year, month,
        )
