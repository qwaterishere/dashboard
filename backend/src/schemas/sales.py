...
class Period(BaseModel):
    label: str = Field(
        description="Готовая подпись периода («02.06 — 01.07.2026»); "
        "предпочтительно форматировать на клиенте из dateFrom/dateTo",
    )
    note: str = Field(
...
class SalesPosition(BaseModel):
    ...
    # Средние на порцию (совместимость): price = revenue/qty, unitCost = cost/qty.
    # Клиент предпочитает суммы revenue/cost.
    price: float = Field(
