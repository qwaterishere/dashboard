...
describe('analytics-cache-key', () => {
  ...
  it('builds latest, month and year keys with tenant scope', () => {
    expect(buildDashboardCacheKey(tenant, {})).toBe('user-1:v11:latest');
    expect(buildDashboardCacheKey(tenant, { year: 2026, month: 6 })).toBe('user-1:v11:m:2026-6');
    expect(buildDashboardCacheKey(tenant, { year: 2025 })).toBe('user-1:v11:y:2025');
  });
  ...
  it('builds week overlay keys', () => {
    ...
    ).toBe('user-1:v11:m:2026-6:w:2026-06-08:2026-06-14');
  });
  ...
  it('appends compare suffix when compare range is set', () => {
    ...
    ).toBe('user-1:v11:latest:c:2026-05-01:2026-05-11');
    ...
    ).toBe('user-1:v11:m:2026-6:c:2026-05-01:2026-05-31');
  });
  ...
  it('extracts tenant scope from cache key', () => {
    expect(tenantScopeFromCacheKey('user-1:v11:m:2026-6')).toBe('user-1');
  });
