...
describe('dashboard-chart.utils', () => {
  it('dashboardChartCacheKey encodes year and month modes with tenant scope', () => {
    ...
    expect(dashboardChartCacheKey('user-1', selection, 'month')).toBe('user-1:m:2026-5');
    expect(dashboardChartCacheKey('user-1', selection, 'year')).toBe('user-1:y:2026');
  });
  ...
  it('chartFetchNeeded is false when selection matches base period in month mode', () => {
    ...
    expect(chartFetchNeeded(null, 'month', basePeriod)).toBe(false);
  });

  it('chartFetchNeeded is always true in year mode', () => {
    expect(chartFetchNeeded(null, 'year', basePeriod)).toBe(true);
  ...
  it('mergeDashboardChartData uses KPIs from chart slice', () => {
    ...
    const merged = mergeDashboardChartData(basePayload, {
      ...
      compare: { year: 2025, month: 5, dayFrom: 1, dayTo: 31 },
      kpis: mayKpis,
  ...
  it('applyWeekTimeframeKpis aggregates default week instead of month KPIs from API', () => {
    const extended = {
      ...basePayload,
      kpis: {
        revenue: { value: 999, prevValue: 90, forecast: 200 },
        checks: { value: 99, prevValue: 9, forecast: 20 },
        guests: { value: 88, prevValue: 18, forecast: 40 },
        avgCheck: { value: 10, prevValue: 9, forecast: 11 },
      },
      revenueByDay: [
        { day: 8, weekday: 1, revenue: 100, checks: 2, guests: 4, plan: null },
    ...
    const adjusted = applyWeekTimeframeKpis(extended);
    expect(adjusted.kpis.revenue.value).toBe(300);
    expect(adjusted.kpis.checks.value).toBe(5);
    expect(adjusted.kpis.revenue.prevValue).toBeNull();
  });

  it('applyWeekTimeframeKpis aggregates selected week within month', () => {
    const extended = {
      ...basePayload,
    ...
    const adjusted = applyWeekTimeframeKpis(
      extended,
      { startDate: '2026-06-08', endDate: '2026-06-14' },
      undefined,
    );
    expect(adjusted.period).toEqual({ year: 2026, month: 6, dayFrom: 8, dayTo: 14 });
    expect(adjusted.kpis.revenue.value).toBe(7700);
    expect(adjusted.kpis.checks.value).toBe(7);
  });
  ...
  it('isChartSliceReady is true for current year interim YTD from latest', () => {
    expect(
      isChartSliceReady(
        basePayload,
        { year: 2026, month: 1 },
  ...
  it('isChartSliceReady is false while another month slice is loading', () => {
    expect(
      isChartSliceReady(
        basePayload,
        { year: 2026, month: 5 },
