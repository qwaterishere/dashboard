...
## Сервер «не запускается» / не отвечает (типовой случай, ловили 18–19.07)

Симптом: uvicorn стартует, но `/health` молчит (таймаут) — ЛИБО повторный
запуск падает с «address already in use». Причина почти всегда одна:
**зомби-uvicorn прошлых дней** (терминал закрыли крестиком — процесс
выжил) держит блокировку SQLite, и новый сервер вечно ждёт её
на `create_all` в старте.

```powershell
# кто держит порт:
Get-NetTCPConnection -LocalPort 8000 -State Listen |
  Select-Object OwningProcess
# все python-процессы с датой старта — зомби видно по СТАРОЙ дате:
Get-Process python | Select-Object Id, StartTime
# снять зомби (СВОЙ сегодняшний сервер не трогать):
Stop-Process -Id <старые pid> -Force
```

После снятия зомби висящий сервер дозагружается сам, перезапуск
не нужен. Профилактика: останавливать сервер Ctrl+C, не закрытием
терминала.
